...
function createApp(
  ...
  app.post("/convert-to-webp", async (c: Context) => {
    ...
    try {
      ...
      if (!image || !(image instanceof File)) {
        return c.json(
          { message: constantConvert.status.NO_IMAGE_IS_PROVIDED.message },
          400,
        );
      }
      console.log("[api] image.size", image.size);
      // if the image file is empty
      if (image.size === 0) {
        return c.json(
          { message: constantConvert.status.INVALID_IMAGE.message },
          400,
        );
      }
      ...
      console.log("[api] convert-to-webp - đọc file thày Uint8Array");

      // Chuyển đổi sang WebP với sharp
      const webpBuffer = await sharp(arrayBuffer)
        .webp({ quality: 80 }) // quality từ 1-100
        .toBuffer();
      console.log("[api] convert-to-webp - chuyển đổi sang WebP với sharp");
      ...
      return c.json({
        message: constantConvert.status.ERROR_IMAGE_PROCESSING.message,
      }, 500);
    }
  });
  ...
  app.post("/convert-to-webp-file", async (c: Context) => {
    try {
      ...
      const sizeCalculatorRs = imageSize(uint8Array);
      console.log(
        "[api] sizeCalculatorRs",
        sizeCalculatorRs.width,
        "- ",
        sizeCalculatorRs.height,
      );
      {
        const { width, height } = sizeCalculatorRs;
        if (width * height > customConfig.limitInputPixels) {
          return c.json({
            status: constantConvert.status.EXCEEDS_PIXEL_LIMIT.code,
            message: constantConvert.status.EXCEEDS_PIXEL_LIMIT.message,
          }, 400);
        }
      }
      // Chuyển đổi sang WebP với sharp
      const webpBuffer = await sharp(uint8Array, {
        limitInputPixels: customConfig.limitInputPixels,
      })
        .webp({ quality: 50 })
        .toBuffer();
      ...
      console.log("[api] trả về file", newFileName);
